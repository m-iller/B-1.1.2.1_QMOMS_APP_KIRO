...
def _to_response(hc) -> HaulCycleResponse:
    return HaulCycleResponse(
        id=str(hc.id),
        machine_id=str(hc.machine_id),
        origin_zone_id=str(hc.origin_zone_id),
        destination_zone_id=str(hc.destination_zone_id),
        payload_tonnes=hc.payload_tonnes,
        status=hc.status,
        immutable=hc.immutable,
...
async def create(payload: CreateHaulCycleRequest, db: AsyncSession) -> HaulCycleResponse:
    hc = await haul_cycle_repository.create_haul_cycle(
        ...
        payload_tonnes=payload.payload_tonnes,
